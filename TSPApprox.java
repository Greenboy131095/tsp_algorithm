 ...
 * order.
 * 
 * @author Katherine (Raven) Russell and KIM DUONG
 */
class TSPApprox extends TSPAlg {
	 ...
	 * {@inheritDoc}`
	 */
	// the total cost of city visited
	...
	public void doNextStep() {
		...
		/*
		 * Basic instructions:
		 * - if the current step is STEP_MST, then call prim's MST to get a tree and
		 * color the edges
		 * - if the current step is STEP_DFT, then perform a DFT of the tree to get a
		 * visit order
		 ...
		 * http://jung.sourceforge.net/doc/api/edu/uci/ics/jung/algorithms/shortestpath/
		 * MinimumSpanningForest.html
		 ...
		 * http://jung.sourceforge.net/doc/api/edu/uci/ics/jung/graph/DelegateForest.
		 * html)
		 ...
		 * - to color the edges of the tree in the graph, use colorAllEdges() (see
		 * above)
		...
		if (step == STEP_MST) {
			DelegateForest<City, Flight> delegateForest = new DelegateForest<City, Flight>();
			HashMap<Flight, Double> map = new HashMap<Flight, Double>();
			for (Flight flight : graph.getEdges()) {
				map.put(flight, (double) flight.getCost());
			}
			MinimumSpanningForest spanningTree = new MinimumSpanningForest(graph, delegateForest, getStartingNode(),
					map);
			tree = spanningTree.getForest();
			colorAllEdges(tree, COLOR_ACTIVE_EDGE);

		}
		if (step == STEP_DFT) {
			City currentNode = getStartingNode();
			visitOrder = new ArrayList<City>();
			depthFirstTraversal(tree, currentNode, visitOrder);

			int edgeCost = visitOrder.size();
			for (int k = 0; k < (edgeCost - 1); k++) {
				n += graph.findEdge(visitOrder.get(k), visitOrder.get(k + 1)).getCost();
			}
			n += graph.findEdge(visitOrder.get(edgeCost - 1), currentNode).getCost();
			this.startingCity = currentNode;
			visitOrderCost = n;
	...
	static void depthFirstTraversal(Forest<City, Flight> treeToWalk, City currNode, List<City> dftVisitOrder) {
		...
		int k = treeToWalk.getChildCount(currNode);

		if (k == 0) {
			return;
		}
		Collection<City> listofCity1 = treeToWalk.getChildren(currNode);
		if (listofCity1 == null) {
			return;
		}
		City[] listofCity = listofCity1.toArray(new City[listofCity1.size()]);
		for (City i : listofCity) {

			depthFirstTraversal(treeToWalk, i, dftVisitOrder);
		}
		 ...
		 * used in class)
		 * - the "do something" step should be ***pre-order*** and append the current
		 * node
