...
class TSPPermutations extends TSPAlg {
	...
	List<City> currentListCheck = new ArrayList<City>();
	...
	public void reset(Graph<City, Flight> graph) {
		...
		currentListCheck = new ArrayList<City>();
	}
	...
	public List<City> permutate() {
		...
		if (permutationItr == null) {
			ArrayList<List<City>> listoflistofCity = new ArrayList<List<City>>();
			...
			List<City> listofCitywithoutstartingCity1 = new ArrayList<>();

			for (int i = 0; i < listofCity.size(); i++) {
				for (int j = 0; j < listofCity.size(); j++) {
					if (j != i) {
						listofCitywithoutstartingCity1.add(listofCity.get(j));
					}

				}
				listoflistofCity.add(listofCitywithoutstartingCity1);

				listofCitywithoutstartingCity1 = new ArrayList<>();

			}

			permutationItr = listoflistofCity.iterator();
	...
	public boolean setupNextStep() {
		...
		if (permutationItr == null || (permutationItr != null && permutationItr.hasNext())) {
	...
	public void doNextStep() {
		int totalCost = 0;
		City startingCity1 = null;

		int numElement = currentPermutation.size();

		List<City> currentPermutationCity = currentPermutation;
		for (City i : listofCity) {
			int check = 0;
			for (City j : currentPermutation) {
				if (i.getId() == j.getId()) {
					check++;
				}
			}
		...
		City startofAnotherCity = startingCity1;

		int check_allList = currentPermutation.size();

		while (check_allList != 0) {
			int minimum1 = Integer.MAX_VALUE;
			City shortestDistaneCity = null;
			for (City i : currentPermutationCity) {

				if (startofAnotherCity != null) {
					int check_minumum = graph.findEdge(i, startofAnotherCity).getCost();
					if (check_minumum < minimum1) {
						minimum1 = check_minumum;
						shortestDistaneCity = i;
					}
				}

			}
			startofAnotherCity = shortestDistaneCity;

			totalCost += minimum1;

			check_allList--;
			currentPermutationCity.remove(shortestDistaneCity);

			currentListCheck.add(startofAnotherCity);

		}

		if (startingCity1 != null && startofAnotherCity != null) {
			totalCost += graph.findEdge(startingCity1, startofAnotherCity).getCost();

			/*
			 * if (visitOrderCost == Integer.MAX_VALUE) {
			 * visitOrderCost = totalCost;
			 * visitOrder = currentListCheck;
			 * this.startingCity = startingCity1;
			 * int sizeofvistOrder = visitOrder.size();
			 * for (int k = 0; k < sizeofvistOrder - 1; k++) {
			 * graph.findEdge(visitOrder.get(k), visitOrder.get(k +
			 * 1)).setColor(COLOR_ACTIVE_EDGE);
			 * 
			 * }
			 * }
			 */
			if (totalCost < visitOrderCost) {
				...
				int sizeofvistOrder = visitOrder.size();
				for (int k = 0; k < sizeofvistOrder - 1; k++) {
					graph.findEdge(visitOrder.get(k), visitOrder.get(k + 1)).setColor(COLOR_ACTIVE_EDGE);

				}

				graph.findEdge(startingCity1, visitOrder.get(0)).setColor(COLOR_ACTIVE_EDGE);
				graph.findEdge(startingCity1, visitOrder.get(sizeofvistOrder - 1)).setColor(COLOR_ACTIVE_EDGE);
			}
			currentListCheck = new ArrayList<City>();
		}
		// YOUR CODE HERE
		 ...
		 * the visit order list and ßvisit order cost (from the parent
		 * TSPAlg class)
